...
export function ArchiveBrowserView({ context }: ArchiveBrowserViewProps) {
  ...
  return (
    <AppShell
      ...
      transferDrawer={<TransferDrawer initialTransfers={context.transferQueue} />}
    >
      <section className="grid gap-7">
        {context.path.length === 0 ? <CollectionsSection context={context} /> : null}
        ...
        <section className="grid gap-5">
              ...
              {context.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm text-[var(--color-text-muted)]">{formatDateLabel(item.updatedAt)}</span>
                    ...
                    <div className="flex items-start gap-2 text-[var(--color-text-soft)]">
                      {href ? (
                        ...
                        <button className="rounded-full p-2 hover:bg-[var(--color-surface-low)] hover:text-[var(--color-text)]">
                          <Share2 size={16} />
                        </button>
                      )}
                      <button className="rounded-full p-2 hover:bg-[var(--color-surface-low)] hover:text-[var(--color-text)]">
          ...
          <div className="flex items-center justify-between rounded-[22px] bg-[var(--color-surface-low)] px-5 py-4 text-sm text-[var(--color-text-muted)]">
            <span>All files are securely synced to the Ethereal Archive.</span>
          ...
          {context.collections.map((collection) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <button className="rounded-full p-2 text-[var(--color-text-soft)] opacity-0 transition-opacity group-hover:opacity-100">
                  <MoreHorizontal size={16} />
                </button>
              </div>
              ...
              <div key={item.id} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-[20px] bg-[var(--color-surface)] px-4 py-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-sm text-[var(--color-text-soft)]">
                    {item.owner} · {formatDateLabel(item.updatedAt)}
                  </p>
                </div>
                <button className="rounded-full p-2 text-[var(--color-text-soft)] hover:bg-[var(--color-surface-low)] hover:text-[var(--color-text)]">
                  <MoreHorizontal size={16} />
                </button>
              </div>
            ))}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="rounded-full bg-[var(--color-surface-low)] p-2 text-[var(--color-text-soft)]">
          <MoreHorizontal size={16} />
        </button>
      </div>
      ...
      <div className="rounded-[32px] bg-[var(--color-surface-high)] p-5">
        ...
        <p className="mt-2 text-sm text-[var(--color-text-muted)]">
          {item.kind === "folder" ? "System Folder" : item.fileType?.toUpperCase() ?? "Object"}
        </p>
      </div>

      <DetailRow label="Location" value={item.path.join(" / ") || "My Archive"} />
      <DetailRow label="Owner" value={item.owner} />
      <DetailRow label="Updated" value={formatDateLabel(item.updatedAt)} />
      ...
      <div className="rounded-[24px] bg-[var(--color-secondary-soft)] px-4 py-4 text-sm leading-6 text-[var(--color-text-muted)]">
        This object is part of your personal cloud archive. Actions are mocked today and ready to be wired to the future Go backend.
      </div>
    </div>
  );
}
...
function GhostAction({ icon: Icon, label }: { icon: typeof ArrowUpWideNarrow; label: string }) {
  return (
    <button className="inline-flex items-center gap-2 rounded-full bg-[var(--color-surface-low)] px-4 py-3 text-sm text-[var(--color-text-muted)] shadow-[inset_0_0_0_1px_var(--color-outline)] transition-colors duration-300 hover:text-[var(--color-text)]">
      <Icon size={16} />
      {label}
    </button>
  );
