...
| Locally repaired — CI reliability | Backend CI invoked `govulncheck@latest` while pinning Go 1.25.13. On 2026-09-19, `latest` resolved to `v1.8.0`, which requires Go 1.26, so a fresh pipeline failed before scanning the application. | CI now pins `govulncheck@v1.7.0`, whose module declares Go 1.25 compatibility. The pinned scanner completed under Go 1.25.13 in the Alpine builder and reported zero reachable vulnerabilities; the pushed GitHub Actions rerun remains the final gate. |
...
- 2026-09-19: A fresh local rerun exposed CI drift: `govulncheck@latest` had advanced to `v1.8.0` and no longer built with the workflow's Go 1.25.13 toolchain. Pinning `govulncheck@v1.7.0` restored deterministic compatibility. The full race suite passed in Alpine against fresh PostgreSQL schema v16 and real MinIO, the pinned vulnerability scan reported zero reachable findings, and both production container images built successfully. WebUI unit tests, ESLint, TypeScript, production build, and dependency audit also passed.
- 2026-09-19: A fresh production browser pass on the still-unrepaired deployment completed Google sign-in, archive listing, search, and PDF download. Existing PDF and PNG previews both rendered `object not found` even though the PDF opened from its generated presigned object URL. At a 390-pixel viewport, the menu button had no accessible name and did not expose a navigation dialog. The Shared page still displayed the legacy metadata-placeholder message. These are baseline failures in the pre-PR release; verify the repaired preview and mobile-navigation implementations again after deployment.
- 2026-09-19: Both hardened PRs were merged. WebUI deployment `35458913413` passed and briefly served the nonce CSP/HSTS release, but backend deployment `35458869986` stopped before migration or cutover because every database and SMTP secret was unset. The new WebUI could not use the legacy backend, so production was restored with revert commit `d4b6bf1` and deployment `35459377795`; health and the prior signed-in experience recovered. Backend image `3b25946` was built and published but was not started. Follow-up release gates now fail the backend deploy before image work when required configuration is absent and refuse a WebUI deploy unless `/drive-api/api/ready` returns the hardened `{"status":"ready"}` response. This rollback is availability evidence only; none of the repaired production acceptance boxes are closed.
