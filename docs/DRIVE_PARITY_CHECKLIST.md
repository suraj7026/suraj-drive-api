...
- [ ] Run containers with health checks, read-only filesystems where practical, dropped capabilities, `no-new-privileges`, resource limits, and scanned/pinned base images. Locally repaired for both app containers: non-root runtime users, matching pinned Alpine 3.23 build/runtime for the CGO backend, readiness health checks, read-only roots, bounded writable `/tmp`, all capabilities dropped, `no-new-privileges`, CPU/memory ceilings, and graceful stops. Image digest pinning, ClamAV hardening, scanning, and deployed enforcement remain.
